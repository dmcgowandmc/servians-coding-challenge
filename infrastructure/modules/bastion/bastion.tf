...
data "aws_ami" "servians_test_ami" {
  most_recent = true
  owners      = ["amazon"]
...
resource "aws_launch_configuration" "bastion_launch" {
  name_prefix = "${var.env_prefix}-bastion"
  # iam_instance_profile = aws_iam_instance_profile.bastion_instance_profile.name

  image_id      = data.aws_ami.servians_test_ami.id
  ...
  key_name = aws_key_pair.key_pair_bastion.key_name
