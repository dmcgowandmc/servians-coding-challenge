resource "aws_subnet" "private" {
  ...
  tags = merge(var.default_tags, {
    "Name"                                    = "${var.env_prefix}-subnet-private-${data.aws_availability_zones.available.names[count.index]}"
...
resource "aws_route_table" "private_for_az" {
  ...
  tags = merge(var.default_tags, {
    "Name" = "${var.env_prefix}-private-${data.aws_availability_zones.available.names[count.index]}-rt"
  })
}
...
resource "aws_nat_gateway" "nat" {
  ...
  tags = merge(var.default_tags, {
    "Name" = "${var.env_prefix}-nat-gateway-${data.aws_availability_zones.available.names[count.index]}"
  })
}
