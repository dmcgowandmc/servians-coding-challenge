resource "aws_db_subnet_group" "storage_db_group" {
  name       = "${var.env_prefix}-storage-db-group"
  subnet_ids = var.vpc_subnet_database_ids
...
resource "aws_db_parameter_group" "storage_db_parameter_group" {
  name        = "${var.env_prefix}-storage-db-parameter-group-nonprod"
